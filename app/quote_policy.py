"""Decide, at read time, whether a US quote is a trade price for the current session.
...
- Stream price: realtime while the stream worker's heartbeat is younger than
  US_STREAM_MAX_AGE seconds and the symbol is still subscribed on the same
...
- REST price: tradeable while its trade time is at most US_MAX_QUOTE_AGE old.
...
from .us_session import EXTENDED, clock_session

STREAM_MODE = {'overnight': 'overnight_stream', 'regular': 'trade_stream'}
REST_MODE = {'overnight': 'overnight_rest', 'regular': 'rest'}


def stream_healthy(stream, now=None):
    now = time.time() if now is None else now
    if not stream or not stream.get('connected'):
        return False
    return now - float(stream.get('heartbeat') or 0) <= int(os.getenv('US_STREAM_MAX_AGE', '10'))


def trade_session(q):
    return clock_session(datetime.fromtimestamp(float(q['timestamp']), timezone.utc))


def assess(q, session, stream=None, now=None):
    """Return a copy of a US quote with session state for `session` added.
    ...
    streaming = (q.get('origin') == 'stream' and stream_healthy(stream, now)
                 and q.get('stream_conn') == stream.get('conn')
                 and q['symbol'] in (stream.get('subscribed') or ()))
    q.update(session=session, trade_session=traded, realtime=False)
    if session in ('closed', 'unknown'):
        ...
        q.update(price_mode='cached', session_tradeable=False,
                 stale=bool(q.get('stale')) or age > int(os.getenv('US_MAX_QUOTE_AGE', '1800')))
        ...
        fresh = age <= int(os.getenv('US_MAX_QUOTE_AGE', '1800'))
...
def session_price_mode(session, stream, rest_ok):
    """Market-level price mode for the overview: what the best symbol can get."""
    if session in ('closed', 'unknown'):
        return 'cached'
    if stream_healthy(stream):
...
def rejection(q):
    """User-facing reason a US quote cannot fill an order now, or None."""
    if q.get('session_tradeable', True):
        return None
    session = q.get('session')
    if session == 'overnight':
        return '현재 미국 데이마켓 체결 시세를 확인할 수 없어 주문할 수 없습니다.'
    if session in EXTENDED:
        name = '프리장' if session == 'pre_market' else '애프터장'
        return f'현재 미국 {name} 체결 시세를 확인할 수 없어 주문할 수 없습니다.'
    if session == 'regular':
        return '현재 미국 정규장 체결 시세를 확인할 수 없어 주문할 수 없습니다.'
    return '현재 미국 주식시장이 장마감 상태이므로 주문할 수 없습니다.'
