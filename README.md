...
1. 영문·숫자·밑줄 3~32자 이름, **8~128자 비밀번호**로 회원가입합니다. 기본 지급액은 가상 USD 100,000, KRW 0입니다.
2. 시장 탐색에서 국내주식·해외주식·국내채권 ETF·해외채권 ETF·금 ETF를 고르고 거래대금·급상승·급하락·ASTER 인기를 선택합니다. 버튼을 누르면 바로 갱신되며 화면을 보고 있는 동안 30초마다 다시 조회합니다. 회사명 또는 코드로도 검색할 수 있습니다. 표시 통화는 거래 통화·KRW·USD 중 선택할 수 있지만 주문은 종목의 원래 통화로 처리됩니다.
3. 종목을 누르면 차트와 주문 패널이 함께 열립니다. 정수 주 단위이며 `최대`는 수수료 포함 수량을 서버에서 계산합니다. 시장가 최대 주문은 **체결 직전 잠금 안에서 다시 계산**하므로 표시 수량과 실제 수량이 달라질 수 있습니다.
...
6. 지정가 주문과 오래된 미국 시세 때문에 대기 중인 시장가 주문은 거래내역의 대기 목록에서 취소할 수 있습니다. 잔액을 예약하지 않으며 실행 시 부족하면 거절됩니다. 시세 장애 시 대기합니다.

한국 종목은 `KR:005930` 형식입니다. 국내 이름 검색은 등록 카탈로그 범위이며, 그 밖의 종목은 6자리 코드를 입력합니다. 채권·금은 미국/한국 상장 ETF로 지원합니다(TLT, IEF, SHY, KR:114260, GLD 등). **개별 채권, 금 현물, 채권 만기·이표 지급 모델은 지원하지 않습니다.**
...
앱 시작 시 기존 테이블을 삭제하지 않고 스키마를 확장합니다. migration은 PostgreSQL transaction과 advisory lock으로 보호하며 `schema_migrations`에 적용 버전(현재 4)을 기록합니다. 재시작 시 같은 변환을 반복하지 않습니다.
...
| `QUOTE_TTL`, `MAX_QUOTE_AGE`, `US_MAX_QUOTE_AGE` | `15`초 캐시, 국내 `900`초·미국 `1800`초까지 즉시 체결; 미국의 더 오래된 시세는 주문 대기 |
...
2. **실전 서버용** App Key와 App Secret을 발급받습니다. ASTER는 실전 KIS 시세 서버에 읽기 전용으로 접속하지만 증권사 주문·잔고 API와는 연결하지 않습니다. KIS 자체 모의투자 서버용 키는 이 어댑터의 서버 주소와 다릅니다.
...
- **KIS 공식 Open API**: [한국투자증권 공식 예제](https://github.com/koreainvestment/open-trading-api/tree/main/examples_llm/domestic_stock)의 당일분봉/기간별시세/거래량순위/등락률순위/휴장일 API를 사용합니다. 실전 시세 서버용 앱 키를 사용하되 주문 API는 구현하지 않았습니다. 시세는 분봉 종가와 실제 영업일·체결시각을 사용하며 초단위 실시간 스트림이 아닙니다. 국내 장 상태는 휴장일 응답+Asia/Seoul 표준시간표로 추정하고 특별 개장시간 미반영을 표시합니다. 국내 이름 전체검색은 종목 마스터 동기화 전까지 제한됩니다.
...
종목 상세와 탐색 화면에서30초마다 backend quote/순위를 다시 확인합니다. 누적 수익률 랭킹은 한국 시간 기준 매시 `00분·30분` 경계에서만 새 스냅샷을 만들며, 한국·미국 시장이 모두 휴장/마감이면 마지막 스냅샷을 유지합니다. 이는 실시간 스트리밍이 아니며 가격 자체는 공급자 지연·시장 시간·캐시에 좌우됩니다. 서버 캐시는 quote15초, 검색5분, 당일차트60초, 기타차트15분, 국내순위120초, 미국순위1시간, 환율1시간, 장상태60초/국내휴장일1일입니다. `TTLCache`가 동시 동일 요청을 합칩니다. **web worker는1개**를 유지하세요. 여러 web 복제본으로 확장할 때는 캐시·요청예산·rate limit을 Redis/PostgreSQL로 옮겨야 합니다.

ASTER 인기는 최근24시간( API는1시간도 지원)의 상세조회·단일결과검색·주문·관심등록 이벤트 합계입니다. 사용자/종목/행동/시간당1회만 기록하고 요청 제한을 적용합니다. 외부 시장의 인기 순위가 아닙니다. 대량 허위 계정까지 막는 강한 부정행위 방지는 별도 과제입니다.

미국 시세는 공급자 지연을 고려해 최대 30분(설정 가능)까지 즉시 체결에 사용합니다. 그보다 오래된 실제 가격이 있으면 시장가 주문을 대기시켜 다음 유효 시세가 들어온 후 체결합니다. 이전 가격으로 가짜 체결하지 않습니다. 주말이나 장 휴무 중에는 오래 대기할 수 있으므로 사용자가 대기 목록에서 취소할 수 있습니다. 실제 거래소 매칭/호가·유동성·슬리피지·장전후 전용 가격은 재현하지 않습니다. 장 상태는 안내 정보이며 거래소와 같은 체결 자격 판정이 아닙니다.
...
### 종목 상세와 수익률 표시
종목 상세의 현재가/등락률은 상승 빨강, 하락 파랑입니다. 1D 차트는 전일 대비 방향, 장기 차트는 표시 구간 첫 종가 대비 마지막 종가 방향으로 색을 지정합니다. 주문 패널은 고정 추천 목록 대신 최근 본 8개 종목을 로그인 아이디별 브라우저 저장소에 저장합니다. 다른 기기에는 동기화되지 않습니다.
...
- ASTER(아스터)는 기존 Paper Harbor의 새 서비스 이름입니다. DB/세션/계정과 프로젝트 경로는 유지합니다.
- 시장 탐색은 코드·데이터 상태·시세시각 열을 제거하고 모바일에서 종목·현재가·등락률·거래대금을 우선 표시합니다. 상세 화면에는 출처와 시각을 유지합니다.
