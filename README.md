...
- 수익률 기준 재설정은 자산과 거래기록을 유지하고 현재 총자산을 0% 기준으로 설정합니다. 전체 초기화는 로그인 계정·비밀번호와 관리자 감사/복구 자료만 유지하고 거래·환전·주문·이체·관심종목·인기 활동·기존 주간 참여 기록을 제거한 뒤 초기 USD를 지급합니다. 계정 삭제는 연결 데이터를 영구 제거하며 현재 로그인한 관리자 자신은 삭제할 수 없습니다.
...
Migration v5는 `wallet_transfers`와 사용자 기록 표시 시작시각을 추가합니다. 전체 초기화·계정 삭제 시 대상 사용자가 당사자인 과거 가상 이체 기록도 제거합니다. 실제 돈을 보내는 기능은 없습니다. Migration v6는 거래·주문 조회용 복합 인덱스를 추가합니다.
