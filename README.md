...
6. 새로운 지정가 주문 UI와 생성 API는 제거했습니다. 기존 지정가 주문 데이터는 삭제하지 않고 원래 조건으로 처리하며 거래내역의 `이전 지정가 주문 기록`에서 조회·취소합니다. 오래된 시세 때문에 대기 중인 시장가 주문도 취소할 수 있습니다. 자금을 예약하지 않으며 실행 시 잔액·보유량이 부족하면 거절됩니다.
