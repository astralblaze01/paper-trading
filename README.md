...
- **티어**: 자산 순위 비율로 정합니다. 마스터 상위 2% · 다이아몬드 5% · 플래티넘 20% · 골드 45% · 실버 75% · 브론즈 나머지. 각 경계는 올림하고 위 티어보다 최소 한 자리 뒤이므로, 인원이 적어도 상위 티어가 비지 않습니다(10명: 마스터 1 · 다이아몬드 1 · 플래티넘 1 · 골드 2 · 실버 3 · 브론즈 2). solved.ac처럼 랭킹의 이름 앞에 티어 아이콘(`app/static/tiers/icons/*.svg`)이 붙고 이름이 티어 색으로 표시되며, 프로필 사진에는 티어 색 테두리와 아이콘이 붙습니다. 티어는 랭킹이 다시 계산될 때마다(장 중 10초 단위) 함께 바뀝니다.
