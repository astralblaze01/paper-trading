...
### 수익률
- **계좌와 종목의 차이**: 계좌 누적 수익률은 원화 기준으로 환율·매매·환전 비용을 포함합니다. 보유 종목 수익률은 거래통화의 평균 매입가 대비 현재가 변화입니다. 표시 통화를 바꿔도 수익률 기준은 바뀌지 않습니다.
  공개·본인 포트폴리오에 기준 원금, 초기 달러 원금의 환율 효과, 그 외 손익을 함께 표시합니다. 이 환율 효과는 초기 원금에 대한 비교값이며 전체 거래의 실현 환차손익을 뜻하지 않습니다.
