...
- **테마**: 로고 색에 맞춘 기본 테마 **딥 틸**(짙은 청록 + 민트 포인트)과 **다크 아레나**(중립 차콜 배경 + 절제된 청록 포인트) 다크 모드. 헤더의 ☾/☀ 버튼으로 바꾸며, 선택은 브라우저에 저장됩니다(처음에는 기기의 다크 모드 설정을 따름). 이익 빨강·손실 파랑은 두 테마 모두 유지합니다. 색은 `app/static/style.css`의 `:root` 변수에서 한 번에 바꿀 수 있습니다.
