...
`KRW 현금 + USD 현금 × USDKRW + 국내 주식 평가액 + 미국 주식 평가액 × USDKRW`가 총자산입니다. 포트폴리오·공개 포트폴리오·누적 랭킹의 수익률은 모두 `(현재 KRW 총자산 - 외부 입출금 누계) / 고정된 초기 KRW 평가액 - 1`을 사용합니다. 따라서 환율 변동도 수익에 포함되며 관리자 지원금(과 이체 기능이 있던 시기의 과거 이체 원금)은 수익에서 제외됩니다.
...
- **환율**: [Frankfurter](https://frankfurter.dev/)의 ECB 일별 KRW→USD 기준환율을 역산합니다. 키가 필요 없고 실시간 환전 호가가 아닙니다. 30분마다 Redis/공급자 값을 확인하며 기준일7일 초과/미래 환율을 거부합니다. API 오류 시 영구적으로 오래된 값을 최신 환율처럼 사용하지 않습니다.
...
관리자 메뉴: DB·Redis·공급자 상태와 데이터 건수, 사용자 검색/활성상태, 신규 초기 지급액, 전체 일반 사용자 지원금, 수익률 기준 재설정, 회원가입 직후 상태 초기화, 계정 삭제, 감사 기록. 전체 초기화는 `CLEAR 사용자이름`, 삭제는 `DELETE 사용자이름`, 전체 지급은 `ALL USERS` 확인 문구가 필요합니다.
...
- 관리자 전용 화면에서 사용자 검색, 잔액/상태 조회, 정지/활성화, 초기자금 설정, 개별·전체 지원금 지급, 수익률 기준 재설정, 기록 전체 초기화, 계정 삭제, 감사 기록 조회를 제공합니다. 전체 대상 작업은 활성 일반 사용자 지원금에만 허용됩니다.
- 수익률 기준 재설정은 자산과 거래기록을 유지하고 현재 총자산을 0% 기준으로 설정합니다. 전체 초기화는 로그인 계정·비밀번호와 관리자 감사/복구 자료만 유지하고 거래·환전·주문·관심종목·인기 활동·기존 주간 참여 기록을 제거한 뒤 초기 USD를 지급합니다. 계정 삭제는 연결 데이터를 영구 제거하며 현재 로그인한 관리자 자신은 삭제할 수 없습니다.
...
### 사용자 간 가상 이체 (제거됨)
사용자 간 가상 이체 기능은 제거되었습니다. 이체 메뉴·화면과 `/api/transfers*`, `/api/users/suggest`, `/api/wallets` API가 없습니다. 이미 반영된 과거 이체는 잔액과 수익률 계산(외부 입출금 누계)에 그대로 남아 있고, `wallet_transfers` 테이블의 기록도 보존합니다. 계정 삭제 시에는 해당 사용자 쪽만 분리(NULL)하며, 전체 초기화 시에는 복구 자료에 보관합니다. Migration v6는 거래·주문 조회용 복합 인덱스를 추가합니다.
