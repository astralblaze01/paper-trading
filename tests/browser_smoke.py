...
with sync_playwright() as p:
    ...
    expect(user.locator('#siteNotice')).to_be_hidden()
    ...
    expect(page.locator('#noticeStatus')).to_contain_text('게시 중 · 서버 점검 예고')
    expect(page.locator('#siteNotice')).to_be_hidden()
    # No reload and no manual refresh: the open page picks the notice up by itself.
    expect(user.locator('#siteNotice')).to_be_visible(timeout=15000)
    expect(user.locator('#siteNotice .notice-new')).to_be_visible()
    expect(user.locator('#toasts')).to_contain_text('새 공지가 등록되었습니다.')
    expect(user.locator('#siteNotice')).to_contain_text('서버 점검이 예정되어 있습니다')
    expect(user.locator('#siteNotice .notice-kind')).to_have_text('서버 점검 예고')
    expect(user.locator('#siteNotice')).to_be_in_viewport()
    assert user.evaluate("api('orders',{symbol:'AAPL',side:'buy',quantity:1,request_id:uuid()}).then(r=>r.quantity)")==1
    user.locator('#siteNotice').click()
    expect(user.locator('#siteNotice .notice-new')).to_be_hidden()
    user.reload(); expect(user.locator('#siteNotice')).to_be_visible()
    expect(user.locator('#siteNotice .notice-new')).to_be_hidden()  # already seen in this browser
    ...
    page.locator('#noticePost').click()
    expect(page.locator('#noticeStatus')).to_contain_text('일반 공지 · 이벤트 안내')
    expect(user.locator('#siteNotice .notice-title')).to_have_text('이벤트 안내',timeout=15000)
    expect(user.locator('#siteNotice .notice-new')).to_be_visible()
    expect(user.locator('#siteNotice .notice-body')).to_have_text('이번 주 수익률 1위에게\n가상 지원금을 드립니다.')
    expect(user.locator('#siteNotice .notice-kind')).to_have_text('일반 공지')
    ...
    expect(user.locator('#siteNotice')).to_be_hidden(timeout=15000)
    user.close()
